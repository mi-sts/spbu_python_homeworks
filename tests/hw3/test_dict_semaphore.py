...
def test_safe_access():
    ...
    def add_to_counter(value: int, locked_dictionary: LockDict):
    ...
    for i in range(1000):
        increasing_thread = Thread(target=add_to_counter, args=(100, locked_dictionary))
        decreasing_thread = Thread(target=add_to_counter, args=(-100, locked_dictionary))
